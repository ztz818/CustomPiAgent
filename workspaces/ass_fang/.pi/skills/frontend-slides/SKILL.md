...
After delivery, **ask the user:** _"Would you like to share this presentation? I can publish it through the built-in HTML render service or export it as a PDF."_

Options:

- **Publish to built-in HTML render service** — Public link that works on any device
...
### 6A: Publish to Built-in HTML Render Service

Do **not** deploy to Vercel, Netlify, view-upload, or any external hosting service. This workspace uses CustomPiAgent's built-in HTML distribution service.

**Required endpoint:**

```text
POST http://127.0.0.1:13000/api/html/upload
GET  http://127.0.0.1:13000/api/html/render/{projectId}
```

**Public URL format:**

```text
http://frpv2k.itgo.cc:11881/api/html/render/{projectId}
```
...
1. Upload `index.html` and every local CSS/JS/image dependency referenced by the HTML.
2. Rewrite local file paths to project-relative paths before uploading.
   - Good: `href="assets/style.css"`
   - Bad: `href="../../../.pi/skills/.../style.css"`
   - Bad: `src="/workspace/.../image.png"`
3. Return both URLs to the user:
   - Local: `http://127.0.0.1:13000/api/html/render/{projectId}`
   - Public: `http://frpv2k.itgo.cc:11881/api/html/render/{projectId}`
4. Verify after upload:
   - HTML returns `200 text/html`
   - CSS returns `200 text/css`
   - JS returns `200 application/javascript`
   - Images return proper image content types
5. Temporary upload scripts must be written to `drafts/YYMMDD/`, never inside the project output folder.

**Recommended projectId:** use a stable, readable slug based on tenant + project + date, for example:

```text
fang_labor_training_260529
```

**Deployment gotchas:**

- Local assets must travel with the HTML. If a page looks broken, inspect `<link>`, `<script>`, `<img>`, and CSS `url(...)` references.
- Prefer generating a self-contained folder structure before uploading: `index.html`, `assets/style.css`, `assets/app.js`, `assets/images/...`.
- Do not use `scripts/deploy.sh` unless the user explicitly asks for external deployment.
...
| [scripts/deploy.sh](scripts/deploy.sh)             | External deployment helper; do not use by default in this workspace   | Only if user explicitly asks for external hosting |
