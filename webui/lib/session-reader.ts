...
import { getAuthorizedWorkspaces, ensureWorkspaceScaffold } from "./workspace-config";
...
async function loadAllSessions(userId?: string): Promise<SessionInfo[]> {
  ...
  return piSessions.map((s) => {
    ...
    getPathToIdCache().set(sessionPathKey(s.path), s.id);
...
export async function listAllSessions(userId?: string): Promise<SessionInfo[]> {
  ...
  if (!userId && globalThis.__piSessionListCache && Date.now() - globalThis.__piSessionListCache.ts < SESSION_LIST_CACHE_TTL_MS) {
    return globalThis.__piSessionListCache.data;
  }

  // Coalescing dedup: concurrent callers share the same in-flight promise
  // only while it belongs to the current cache generation.
  if (!userId && globalThis.__piSessionListPromise && globalThis.__piSessionListPromiseGeneration === generation) {
  ...
  const loadPromise = loadAllSessions(userId).then((data) => {
    ...
    if (!userId && (globalThis.__piSessionListGeneration ?? 0) === generation) {
