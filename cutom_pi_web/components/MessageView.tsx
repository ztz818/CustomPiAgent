...
function AssistantMessageView({
  ...
  return (
    <div
      style={{ marginBottom: 18, padding: "2px 0" }}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      {/* Model label */}
  ...
  return (
    <div
      style={{
        ...
        overflow: "hidden",
        fontSize: 12,
        border: isError ? "1px solid color-mix(in srgb, var(--error) 40%, var(--outline))" : "1px solid var(--outline-variant)",
        background: isError ? "color-mix(in srgb, var(--error) 7%, var(--surface))" : "var(--surface-container)",
      }}
    >
      {/* ── Tool call header ── */}
      <button
      ...
      {expanded && (
        <pre
          style={{
            ...
            overflow: "auto",
            background: "var(--surface)",
            borderTop: isError ? "1px solid color-mix(in srgb, var(--error) 24%, var(--outline))" : "1px solid var(--outline-variant)",
            whiteSpace: "pre-wrap",
            wordBreak: "break-all",
          }}
        >
...
function PairedResult({ text, isEmpty, isError }: {
  ...
  return (
    <div
      style={{
        borderTop: `1px solid ${isError ? "color-mix(in srgb, var(--error) 28%, var(--outline))" : "var(--outline-variant)"}`,
        background: isError ? "color-mix(in srgb, var(--error) 6%, var(--surface))" : "var(--surface)",
      }}
    >
      <pre
        style={{
